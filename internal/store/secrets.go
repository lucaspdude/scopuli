...
func (s *Store) PutSecret(ctx context.Context, sec *Secret) error {
	...
	// LastInsertId is unreliable on the UPDATE arm of an upsert: it returns
	// the rowid of the most recent plain INSERT on this connection (e.g. an
	// audit row), not the updated row. Fetch the real id by path instead.
	if err := s.db.QueryRowContext(ctx,
		`SELECT id FROM secrets WHERE path = ?`, sec.Path,
	).Scan(&sec.ID); err != nil {
		return err
	}
	// FTS sync: the FTS5 tables are regular content mode (no triggers), so
	// sync explicitly after every write.
