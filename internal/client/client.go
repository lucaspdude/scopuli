...
// ListSecrets issues GET /api/secrets.
func (c *Client) ListSecrets(prefix string) ([]map[string]any, error) {
	path := "/api/secrets"
	if prefix != "" {
...
// envOrKeyring prefers env var SCOPULI_URL/SCOPULI_KEY.
func envOrKeyring() (*Client, error) {
	if url := os.Getenv("SCOPULI_URL"); url != "" {
		tok := os.Getenv("SCOPULI_KEY")
		if tok == "" {
			return nil, errors.New("SCOPULI_URL set but SCOPULI_KEY missing")
		}
