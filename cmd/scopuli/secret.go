...
func newSecretSetCmd() *cobra.Command {
	var label, description, value string
	var tags, meta, metadata []string
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			val, err := resolveValue(value, valueFromStdin, valueFromFile)
			if err != nil {
				return err
			}
			// Accept either --meta or --metadata (they're the same flag).
			combined := append(meta, metadata...)
			req := map[string]any{
				"path":        args[0],
				"value":       val,
				"label":       label,
	...
	cmd.Flags().StringVar(&value, "value", "", "plaintext value (or use --value-from-stdin / --value-from-file)")
	...
	cmd.Flags().StringSliceVar(&metadata, "metadata", nil, "alias of --meta")
...
func newSecretListCmd() *cobra.Command {
	var prefix string
	var asJSON, reveal bool
	cmd := &cobra.Command{
		Use:   "list",
		Short: "List secrets (masked value preview, tags, description)",
		RunE: func(cmd *cobra.Command, _ []string) error {
			c, err := client.EnvOrKeyring()
			if err != nil {
				return err
			}
			out, err := c.ListSecrets(prefix, reveal)
			if err != nil {
				return err
			}
			if asJSON {
			...
			fmt.Fprintln(tw, "PATH\tVALUE\tTAGS\tDESCRIPTION")
			for _, it := range out {
				path, _ := it["path"].(string)
				tags, _ := it["tags"].(string)
				desc, _ := it["description"].(string)
				val, _ := it["value_preview"].(string)
				if reveal {
	...
	cmd.Flags().BoolVar(&reveal, "reveal", false, "show full plaintext values")
	return cmd
}
...
func resolveValue(value string, stdin, file bool) (string, error) {
	// If multiple sources are given, stdin's value wins (most likely to be
	// what the user actually meant since it's the most explicit).
	if stdin && file {
	...
	if count > 1 {
		return "", fmt.Errorf("--value, --value-from-stdin, --value-from-file are mutually exclusive")
	}
	if stdin {
